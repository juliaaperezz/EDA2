...
typedef struct User {
    ...
    int postCount;
    int friendCount;
    struct User *friendList[MAX_USERS];
    struct User *friendRequests[MAX_USERS];
    int pendingRequests;
} User;
...
void insertUser(UserList *list) {
    ...
    newUser.pendingRequests = 0;
    //User *friendList = NULL;   // creo que no se usa
...
void operateAsUser(UserList list) {
    ...
    do {
        ...
        printf("6. Connect with a stranger\n");
...
void addFriend(UserList list, User *user) {
    ...
    for (int i = 0; i < user->friendCount; i++) {
        if (strcmp(friend->username, user->friendList[i]->username) == 0) {
    ...
    for (int i = 0; i < user->pendingRequests; i++) {
        if (strcmp(friend->username, user->friendRequests[i]->username) == 0) {
    ...
    if (user->pendingRequests >= MAX_USERS) {
        printf("Maximum number of pending friend requests reached.\n");
        return;
    }

    friend->friendRequests[friend->pendingRequests++] = user;  // el último elemento de las solicitudes del amigo es el user(el que opera como usuario)
    friend->pendingRequests++;    // actualizar pendingrequest
    ...
    for (int i = 0; i < list.count; i++) {
        ...
        if (currUser != user) {
            ...
            for (int j = 0; j < user->friendCount; j++) {
                if (strcmp(currUser->username, user->friendList[j]->username) == 0) {
                    isFriend = 1;
                    break;  //sospechoso
                }
            }
...
void handleFriendRequests(User *user) {
    ...
    for (int i = 0; i < user->pendingRequests; i++) {
        ...
        printf("Age: %d\n", user->friendRequests[i]->age);
        printf("Email: %s\n", user->friendRequests[i]->email);
        printf("Location: %s\n", user->friendRequests[i]->location);
        printf("==============================\n");

        printf("Do you want to accept the friend request from %s? (y/n): ", user->friendRequests[i]->username);
        char choice;
        scanf(" %c", &choice);

        if (choice == 'y' || choice == 'Y') {
            // Add the friend to the user's friend list
            strcpy(user->friendList[user->friendCount++], user->friendRequests[i]); //user->friendRequests[i]-> username
            //user->friendCount++;    // actualizar contador user friendList
            printf("Friend request accepted. %s added to your friend list.\n", user->friendList[user->friendCount]->username);  //asi comprobamos que esta en la lista

            // Add the user to the friend's friend list
            strcpy(user->friendList[user->friendCount]->friendList[user->friendList[user->friendCount]->friendCount++], user);
            //user->friendList[user->friendCount]->friendCount++;  //actualizar contadorfriendCount friend
            printf("%s added to %s's friend list.\n", user->username, user->friendRequests[i]->username);

            // Remove the friend request
            for (int j = i; j < user->pendingRequests - 1; j++) {
                user->friendRequests[j] = user->friendRequests[j + 1];
            }
